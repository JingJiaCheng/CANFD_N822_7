...
void heatshrink_encoder_reset(heatshrink_encoder *hse) {
    ...
    hse->match_scan_index = 0u;
    hse->flags = 0u;
    hse->bit_index = 0x80u;
    hse->current_byte = 0x00u;
    hse->match_length = 0u;

    hse->outgoing_bits = 0x0000u;
    hse->outgoing_bits_count = 0u;
...
static HSE_state st_step_search(heatshrink_encoder *hse) {
    ...
    LOG("## step_search, scan @ +%d (%d/%d), input size %d\n",
        msi, hse->input_size + msi, 2u*window_length, hse->input_size);

    bool fin = is_finishing(hse);
    if (msi > hse->input_size - (fin ? 1u : lookahead_sz)) {
    ...
    if (match_pos == MATCH_NOT_FOUND) {
        ...
        ASSERT(match_pos <= 1u << HEATSHRINK_ENCODER_WINDOW_BITS(hse) /*window_length*/);
...
static HSE_state st_yield_br_index(heatshrink_encoder *hse,
    ...
    if (can_take_byte(oi)) {
        ...
        if (push_outgoing_bits(hse, oi) > 0u) {
            return HSES_YIELD_BR_INDEX; /* continue */
        } else {
            hse->outgoing_bits = hse->match_length - 1u;
...
static uint16_t get_input_buffer_size(heatshrink_encoder *hse) {
    return (1u << HEATSHRINK_ENCODER_WINDOW_BITS(hse));
    /*(void)hse;*/
}

static uint16_t get_lookahead_size(heatshrink_encoder *hse) {
    return (1u << HEATSHRINK_ENCODER_LOOKAHEAD_BITS(hse));
    /*(void)hse;*/
}
...
static uint16_t find_longest_match(heatshrink_encoder *hse, uint16_t start,
    ...
    const size_t break_even_point =
      (1u + HEATSHRINK_ENCODER_WINDOW_BITS(hse) +
    ...
    if (match_maxlen > (break_even_point / 8u)) {
...
static uint8_t push_outgoing_bits(heatshrink_encoder *hse, output_info *oi) {
    uint8_t count = 0u;
    uint8_t bits = 0u;
    if (hse->outgoing_bits_count > 8u) {
        count = 8u;
        bits = hse->outgoing_bits >> (hse->outgoing_bits_count - 8u);
    } else {
...
static void push_bits(heatshrink_encoder *hse, uint8_t count, uint8_t bits,
    ...
    if (count == 8u && hse->bit_index == 0x80u) {
...
static void push_literal_byte(heatshrink_encoder *hse, output_info *oi) {
    uint16_t processed_offset = hse->match_scan_index - 1u;
