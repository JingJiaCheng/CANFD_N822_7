...
heatshrink_decoder *heatshrink_decoder_alloc(uint16_t input_buffer_size,
    ...
    size_t buffers_sz = (1u << window_sz2) + input_buffer_size;
    ...
    LOG("-- allocated decoder with buffer size of %zu (%zu + %u + %u)\n",
        sz, sizeof(heatshrink_decoder), (1u << window_sz2), input_buffer_size);
    return hsd;
}

void heatshrink_decoder_free(heatshrink_decoder *hsd) {
    size_t buffers_sz = (1u << hsd->window_sz2) + hsd->input_buffer_size;
    size_t sz = sizeof(heatshrink_decoder) + buffers_sz;
...
void heatshrink_decoder_reset(heatshrink_decoder *hsd) {
    size_t buf_sz = 1u << HEATSHRINK_DECODER_WINDOW_BITS(hsd);
...
static HSD_state st_tag_bit(heatshrink_decoder *hsd) {
    uint32_t bits = get_bits(hsd, 1u);  // get tag bit
    if (bits == NO_BITS) {
        return HSDS_TAG_BIT;
    } else if (bits) {
        return HSDS_YIELD_LITERAL;
    } else if (HEATSHRINK_DECODER_WINDOW_BITS(hsd) > 8u) {
...
static HSD_state st_backref_index_msb(heatshrink_decoder *hsd) {
	...
	uint16_t bits;
    ASSERT(bit_ct > 8u);
    bits = get_bits(hsd, bit_ct - 8u);
    LOG("-- backref index (msb), got 0x%04x (+1)\n", bits);
    if (bits == NO_BITS) { return HSDS_BACKREF_INDEX_MSB; }
    hsd->output_index = bits << 8u;
    return HSDS_BACKREF_INDEX_LSB;
}

static HSD_state st_backref_index_lsb(heatshrink_decoder *hsd) {
    uint8_t bit_ct = BACKREF_INDEX_BITS(hsd);
    uint16_t bits = get_bits(hsd, bit_ct < 8u ? bit_ct : 8u);
	uint8_t br_bit_ct;
    ...
    hsd->output_count = 0u;
    return (br_bit_ct > 8u) ? HSDS_BACKREF_COUNT_MSB : HSDS_BACKREF_COUNT_LSB;
}
...
static HSD_state st_backref_count_msb(heatshrink_decoder *hsd) {
	...
	uint16_t bits;
    ASSERT(br_bit_ct > 8u);
    bits = get_bits(hsd, br_bit_ct - 8u);
    LOG("-- backref count (msb), got 0x%04x (+1)\n", bits);
    if (bits == NO_BITS) { return HSDS_BACKREF_COUNT_MSB; }
    hsd->output_count = bits << 8u;
    return HSDS_BACKREF_COUNT_LSB;
}

static HSD_state st_backref_count_lsb(heatshrink_decoder *hsd) {
    uint8_t br_bit_ct = BACKREF_COUNT_BITS(hsd);
    uint16_t bits = get_bits(hsd, br_bit_ct < 8u ? br_bit_ct : 8u);
...
static HSD_state st_yield_backref(heatshrink_decoder *hsd,
    ...
    if (count > 0u) {
        ...
        ASSERT(neg_offset <= mask + 1u);
        ASSERT(count <= (size_t)(1u << BACKREF_COUNT_BITS(hsd)));
        ...
        if (hsd->output_count == 0u) { return HSDS_TAG_BIT; }
    }
    return HSDS_YIELD_BACKREF;
}
...
static uint16_t get_bits(heatshrink_decoder *hsd, uint8_t count) {
    ...
    uint8_t i = 0u;
    if (count > 15u) { return NO_BITS; }
    LOG("-- popping %u bit(s)\n", count);

    /* If we aren't able to get COUNT bits, suspend immediately, because we
     * don't track how many bits of COUNT we've accumulated before suspend. */
    if (hsd->input_size == 0u) {
        if (hsd->bit_index < (1u << (count - 1u))) { return NO_BITS; }
    }

    for (i = 0u; i < count; i++) {
        if (hsd->bit_index == 0x00u) {
            if (hsd->input_size == 0u) {
            ...
            if (hsd->input_index == hsd->input_size) {
                hsd->input_index = 0u; /* input is exhausted */
                hsd->input_size = 0u;
            }
            hsd->bit_index = 0x80u;
        }
    ...
    if (count > 1u) { LOG("  -- accumulated %08x\n", accumulator); }
    return accumulator;
}
...
HSD_finish_res heatshrink_decoder_finish(heatshrink_decoder *hsd) {
    ...
    switch (hsd->state) {
    case HSDS_TAG_BIT:
        return hsd->input_size == 0u ? HSDR_FINISH_DONE : HSDR_FINISH_MORE;
